...
export async function callDeepSeek(
    ...
    try {
        ...
        if (currentModel.provider === 'google') {
            return await callGoogleApi(currentModel, apiKey, messages);
        }
...
export async function* streamDeepSeek(
    ...
    if (currentModel.provider === 'google') {
        yield* streamGoogleApi(currentModel, apiKey, messages);
        return;
    }
...
// Google API 不支持真正的流式响应，此函数模拟流式输出
async function* streamGoogleApi(
    model: ModelConfig,
    apiKey: string,
    messages: Message[]
): AsyncGenerator<string, void, unknown> {
    ...
    try {
        ...
        // 模拟流式输出，逐字符 yielding（因为 Google API 不支持流式）

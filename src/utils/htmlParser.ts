...
export async function insertHtmlAsDocx(
  ...
  if (formulaMarkers.length > 0) {
    ...
    for (const { marker, tex, isBlock } of formulaMarkers) {
      console.log('[insertHtmlAsDocx] 替换公式:', marker, '→', tex.substring(0, 30));
      ...
      if (searchResults.items.length > 0) {
        ...
        // 统一使用行内 OOXML（块级公式已在独立居中 <p> 中）
        const ooxml = generateInlineMathOOXML(tex);
        ...
        console.log('[insertHtmlAsDocx] 公式替换成功:', marker, `字体: ${mathFont} ${mathFontSize}pt`);
      } else {
        console.warn('[insertHtmlAsDocx] 未找到标记:', marker);
      }
    }
  }
